...
create_table_sql = """
    CREATE TABLE IF NOT EXISTS diamondsSheet (
        ID INT PRIMARY KEY,
...
def sync_data():
    ...
    # making tuple for execute many
    data_tuples =  [(i['ID'], i['Stock id'], i['ReportNo'], i['Shape'], i['Carats'], i['color'], i['clarity'], i['cut'], i['polish'], i['symmetry'], i['flou'], i['length'], i['width'], i['height'], i['depth'], i['table'], i['lab'], i['video'], i['image'], i['Price'], i['Currency']) for i in sheet_data]


    
    sql = f"""
            INSERT INTO diamondsSheet (ID, Stock_id, ReportNo, Shape, Carats, color, clarity, cut, polish, symmetry, flou, length, width, height, depth, `table`, lab, video, image, Price, Currency)
